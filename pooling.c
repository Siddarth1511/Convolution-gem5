        ...
        for(int m=0;m<out->h;m++)
            for(int n=0;n<out->w;n++){
              ...
              for(int k=0;k<Ky;k++)
                    for(int l=0;l<Kx;l++)
                        switch(p->type){
                            case POOL_AVG:r+=blob_data(in,o,m*Sy+k,n*Sx+l);
                  /*             r+=blob_data(in,o,m*Sy+0,n*Sx+0);
				r+=blob_data(in,o,m*Sy+0,n*Sx+1);
				r+=blob_data(in,o,m*Sy+1,n*Sx+0);
				r+=blob_data(in,o,m*Sy+1,n*Sx+1);	
					}*/
                            break;
                            case POOL_MAX:r=fmax(r,blob_data(in,o,m*Sy+k,n*Sx+l));
                              /* r=fmax(r,blob_data(in,o,m*Sy+0,n*Sx+0));
                     	       r=fmax(r,blob_data(in,o,m*Sy+0,n*Sx+1));	   
				r=fmax(r,blob_data(in,o,m*Sy+1,n*Sx+0));	   
